...
@startuml
loop !EOF
Client->Server : SendAll(header+payload)
Server->Server : Store_bets(bet)
Server->Client : return ACK
end
Server->Server: Calculate_Winners
loop winners
Server->Client : SendAll(header+payload)
return ACK
end
Client->Client : Save(winners)
@enduml